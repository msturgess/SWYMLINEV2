...
TArray<float> USerialCom::BytesToFloats(TArray<uint8> Bytes)
{
...
TArray<uint8> USerialCom::FloatsToBytes(const TArray<float>& Floats)
{
	TArray<uint8> Bytes;

	for (int i = 0; i < Floats.Num(); i++)
	{
		Bytes.Append(reinterpret_cast<const uint8*>(&Floats[i]), 4);
	}
